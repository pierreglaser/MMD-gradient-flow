...
class mmd2_noise_injection(autograd.Function):
    ...
    def forward(ctx, true_feature, fake_feature, noisy_feature):
        ...
        with tr.enable_grad():
            ...
            mmd2_for_grad = (n_particles / b_size) * (
                tr.einsum("nd,nd->", fake_feature, mean_noisy_feature)
                - tr.einsum("nd,nd->", true_feature, mean_noisy_feature)
            )
    ...
    def backward(ctx, grad_output):
        ...
        with tr.enable_grad():
            gradients = autograd.grad(
                outputs=mmd2_for_grad,
                inputs=noisy_feature,
...
class mmd2_func(autograd.Function):
    ...
    def forward(ctx, true_feature, fake_feature):
        ...
        with tr.enable_grad():
            ...
            mmd2 = (n_particles / b_size) * tr.sum(
                (true_feature - tr.mean(fake_feature, dim=-1)) ** 2
            )

        ctx.save_for_backward(mmd2, fake_feature)

        return (1.0 / n_particles) * mmd2
...
class sobolev(autograd.Function):
    ...
    def forward(ctx, true_feature, fake_feature, matrix):
        ...
        with tr.enable_grad():
            ...
            mmd2 = (0.5 * n_particles / b_size) * tr.sum(
                (true_feature - tr.mean(fake_feature, dim=-1)) ** 2
            )
            mmd2_for_grad = (1.0 / b_size) * tr.einsum(
                "id,idm->", alpha, fake_feature
            )
...
class Sobolev(nn.Module):
    ...
    def forward(self, x, y):
        ...
        matrix = (1.0 / (num_particles * b_size)) * tr.einsum(
            "im,jm->ij", grad_out, grad_out
        ) + self.lmbda * tr.eye(b_size, dtype=x.dtype, device=x.device)
