...
class NoisyLinear(nn.Linear):
    def __init__(
        ...
        self.register_buffer(
            "epsilon_weight", tr.zeros(out_features, in_features)
        )
        if bias:
    ...
    def forward(self, input):
        if self.noisy_mode:
            ...
            if bias is not None:
                ...
                bias = bias + self.noise_level * Variable(
                    self.epsilon_bias, requires_grad=False
                )
            self.noisy_mode = False
            return F.linear(
                input,
                self.weight
                + self.noise_level
                * Variable(self.epsilon_weight, requires_grad=False),
                bias,
            )
        else:
    ...
    def add_noise(self):
        tr.randn(self.epsilon_weight.size(), out=self.epsilon_weight)
        self.weight.data += self.noise_level * Variable(
            self.epsilon_weight, requires_grad=False
        )
        bias = self.bias
        if bias is not None:
            tr.randn(self.epsilon_bias.size(), out=self.epsilon_bias)
            self.bias.data += self.noise_level * Variable(
                self.epsilon_bias, requires_grad=False
            )
...
class SphericalTeacher(tr.utils.data.Dataset):
    def __init__(self, network, N_samples, dtype, device):
        D = network.d_int
        self.device = device
        self.source = tr.distributions.multivariate_normal.MultivariateNormal(
            tr.zeros(D, dtype=dtype, device=device),
            tr.eye(D, dtype=dtype, device=device),
        )
        source_samples = self.source.sample([N_samples])
        inv_norm = 1.0 / tr.norm(source_samples, dim=1)
